...
const UserSchema = new mongoose.Schema({
  ...
  userType: { type: String, default: 'eventCoordinator' } // Assuming all are students
});

const User = mongoose.model('EventCoordinatorPassword', UserSchema);

// CSV file path
// Go one folder up (`..`) and then into the CSVFILES folder to access PASSWORD.csv
const csvFilePath = path.join(__dirname, '../CSVFILES/EVENTCOORDINATORPASSWORD.csv');
  ...
  try {
    ...
    const processedUsers = users.map(user => {
      return {
        ...
        password: user.Eventcoordinatorrollno, // Use roll number directly without hashing
        userType: 'eventCoordinator' // Set userType as teacher
      };
    });
